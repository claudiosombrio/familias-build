controllers.controller('cadastroDomicilioCtrl', ['$scope', '$state', 'DB', '$stateParams', '$ionicLoading', '$ionicPopup', '$q', '$ionicPlatform', 'temporario', 
    function ($scope, $state, DB, $stateParams, $ionicLoading, $ionicPopup, $q, $ionicPlatform, temporario) {
        ...
        $scope.bind = {individuosExcluidos: []};
        ...
        $scope.salvarIndividuos = function(tx, idDomicilio){
            ...
            if($scope.bind.individuosExcluidos.length > 0){
                for(var i=0; i < $scope.bind.individuosExcluidos.length; i++){
                    ...
                    tx.executeSql("UPDATE paciente SET codigoUsuario = ?, motivoExclusaoDomicilio = ? WHERE id = ?", [$scope.usuario.id, individuo.motivoExclusao, individuo.idIndividuo], null, $scope.err);
                }
            }
            ...
            tx.executeSql("UPDATE paciente SET codigoDomicilio = null, codigoUsuario = ? WHERE codigoDomicilio = ?", [$scope.usuario.id, idDomicilio], function(tx, result){
                ...
                if(isValid($scope.domicilio.individuos) && $scope.domicilio.individuos.length > 0){
                    ...
                    for(var i = 0; i < $scope.domicilio.individuos.length; i++){
                        sql += virgula + "?";
                        param.push($scope.domicilio.individuos[i].id);
                        virgula = ",";
                    }
                    sql += ")";
...
//        $scope.enderecoBind = {};
        ...
        $scope.selecionarTipoLogradouro = function () {
            ...
            $scope.bindTipoLogradouro = {criterioTipoLogradouro: ''};
...
controllers.controller('tabDomIndividuosCtrl', ['$scope', '$ionicModal', 'DB', '$timeout', '$celkScroll', '$state', '$ionicActionSheet', '$ionicPopup',
    function ($scope, $ionicModal, DB, $timeout, $celkScroll, $state, $ionicActionSheet, $ionicPopup) {
        ...
        $scope.removerIndividuo = function (index, motivo) {
            ...
            $scope.$parent.bind.individuosExcluidos.unshift({idIndividuo: i.id, motivoExclusao: motivo});
        ...
        $scope.selecionarIndividuo = function () {
            ...
            $scope.bindIndividuo = {criterioIndividuo: ''};
        ...
        $scope.individuoSelecionado = function (item) {
            if(isValid(item.codigoMicroArea)){
                $scope.showConfirmation('O paciente selecionado pertence a Àrea: '+item.descricaoEquipe+' | MicroÀrea: '+item.microArea+' | Família: '+item.numeroFamilia+'. Deseja remover do domicílio atual?')
        ...
        $scope.loadIndividuo = function () {
            ...
            DB.query("SELECT p.sexo, p.id, p.nome, p.nomeMae, strftime('%d/%m/%Y',p.dataNascimento, 'unixepoch') AS dataNascimento, p.codigoDomicilio, p.situacao, " +
                    " dom.codigoMicroArea, " +
                    " dom.numeroFamilia, " +
                    " e.descricao AS descricaoEquipe, " +
                    " ma.microArea " +
                    ...
                    " LEFT JOIN microArea AS ma ON ma.id = dom.codigoMicroArea " +
                    " LEFT JOIN equipeProfissional AS ep ON ma.id = ep.codigoMicroArea AND ep.status = 0 " +
                    " LEFT JOIN equipe AS e ON e.id = ep.codigoEquipe " +
                    " WHERE p.nome like ? and p.situacao <> 3 ORDER BY p.nome LIMIT ?,?", [filtro, i, deep]).then(function (result) {
                var individuos = DB.fetchAll(result);
        ...
        $scope.openItemOptions = function (itemIndex) {
            if ($scope.verBotaoSalvar) {
                $scope.hideSheet = $ionicActionSheet.show({
                    ...
                    buttonClicked: function (index) {
                        $scope.salvarTemporario();
                        $state.go('cadastroIndividuos.tabIdentificacao', {idIndividuo: $scope.$parent.domicilio.individuos[itemIndex].id});
                        return true;
                    }
                });
            }
        ...
        $scope.showPopupRemover = function (itemIndex) {
            $scope.data = {mudouse: false, morreu: false, outros: false};
            var popupRemover = $ionicPopup.show({
                template: '<li class="item item-checkbox"><label class="checkbox"><input type="checkbox" ng-change="opcoesRemover(\'mudouse\')" ng-model="data.mudouse"></label>Mudou-se</li>'
                        + '<li class="item item-checkbox"><label class="checkbox"><input type="checkbox" ng-change="opcoesRemover(\'morreu\')" ng-model="data.morreu"></label>Morreu</li>'
                        + '<li class="item item-checkbox"><label class="checkbox"><input type="checkbox" ng-change="opcoesRemover(\'outros\')" ng-model="data.outros"></label>Outros</li>',
