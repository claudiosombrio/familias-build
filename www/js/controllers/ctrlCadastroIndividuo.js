controllers.controller('cadastroIndividuoCtrl', ['$scope', '$filter', 'DB', '$stateParams', '$ionicLoading', '$ionicPopup', '$q', '$ionicPlatform', '$state',
    function($scope, $filter, DB, $stateParams, $ionicLoading, $ionicPopup, $q, $ionicPlatform, $state) {
        ...
        $scope.bind.codigoInterno = $stateParams.codigoInterno
        ...
        $scope.voltar = function() {
            window.localStorage.individuoToEdit = '{}'
            var tempVisita = JSON.parse(window.localStorage.getItem("tempVisita"));
            if (tempDomicilio) {
                $state.go('cadastroDomicilios.tabIndividuos');
            } else if (tempVisita) {
                $state.go('cadastroVisitas.tabVisita', {
                    idDomicilio: tempVisita.bind.idDomicilio
                });
            } else {
                $state.go('individuos');
            }
        ...
        function validaGlobal(paciente) {
            if (paciente) {
                ...
                $scope.bind.id = paciente.id;
            }
        };

        $scope.carregarPaciente = function() {
            var editerio = {}
            if (window.localStorage.individuoToEdit && window.localStorage.individuoToEdit !== 'undefined') {
                editerio = JSON.parse(window.localStorage.individuoToEdit)
            } else {
                editerio.codigoInterno = $scope.bind.codigoInterno;
            }
            if ($scope.bind.idIndividuo || editerio.codigoInterno || editerio.codigoSistema) {
                DB.query("SELECT p.id," +
                    ...
                    " p.codigoSistema AS codigoSistema," +
                    ...
                    " WHERE p.id = ? OR p.codigoSistema = ?", [$scope.bind.idIndividuo === "" ? editerio.codigoInterno : $scope.bind.idIndividuo, editerio.codigoSistema]).then(function(result) {
                    ...
                    if (paciente) {
                        ...
                        paciente.codigoInterno = paciente.id;
                        validaGlobal(paciente);
                        $scope.habilitarEtnia();
                        $scope.carregarResponsavel();
                    } else {
                        var sql = "SELECT ativo AS situacao, " +
                            ...
                            "codigoArea AS codigoArea, " +
                            "codigoMicroArea AS codigoMicroArea " +
                            "FROM dominioPaciente " +
                            "WHERE codigoSistema = ?";


                        DB.query(sql, [$scope.bind.idIndividuo === "" ? editerio.codigoSistema : $scope.bind.idIndividuo]).then(function(_result) {
                            paciente = DB.fetch(_result);
                            if (paciente.numeroCartao === "null") {
                                paciente.numeroCartao = null;
                            }
                            $scope.documentosBind.numeroCpf = paciente.cpf
                            paciente.codigoMicroArea = $scope.codigoMicroArea;
                            paciente.id = paciente.codigoInterno;
                            validaGlobal(paciente);
                            $ionicLoading.hide();
                        }, function(err) {
                            $scope.err(null, err);
        ...
        $scope.carregarResponsavel = function() {
            if (isInvalid($scope.paciente.codigoResponsavelFamiliar) && isValid($scope.paciente.codigoSistema)) {
                DB.query("SELECT p.id AS codigoResponsavelFamiliar," +
                    " upper(p.nome) AS nomeResponsavel" +
                    " FROM pacienteResponsavelAux AS res " +
                    "  left outer join paciente AS p ON p.codigoSistema = res.codigoResponsavelSistema " +
                    " WHERE res.codigoPacienteSistema = ?", [$scope.paciente.codigoSistema]).then(function(result) {

                    var responsavel = DB.fetch(result);
                    if (responsavel) {
                        $scope.paciente.codigoResponsavelFamiliar = responsavel.codigoResponsavelFamiliar;
                        $scope.paciente.nomeResponsavel = responsavel.nomeResponsavel;
                    }
        ...
        $scope.carregarPacienteEsus = function() {
            DB.query("SELECT p.id," +
                ...
                " WHERE p.codigoPaciente = ?", [$scope.bind.id]).then(function(result) {
        ...
        $scope.carregarDocumentos = function() {
            DB.query("SELECT d.id," +
                ...
                " WHERE d.codigoPaciente = ? AND excluido = 'N'", [$scope.bind.id]).then(function(result) {
            ...
            try {
                ...
                if (isValid($scope.documentosBind.numeroRg) ||
                    ...
                    if (isInvalid($scope.documentosBind.numeroRg) ||
                        ...
                        throw "Informe todos os campos da Identidade";
                    }
                }
                if (isValid($scope.documentosBind.tipoDocumentoCertidao) ||
                    ...
                    if (isInvalid($scope.documentosBind.tipoDocumentoCertidao) ||
                        ...
                        throw "Informe todos os campos da Certidão";
                    }
                }
                if (invalidToEmpty($scope.documentosBind.numeroCpf).length === 0 && isInvalid($scope.documentosBind.numeroRg) && isInvalid($scope.documentosBind.numeroFolhaCertidao) && $scope.paciente.situacao != 3) {
                    throw "Informe o CPF, RG ou uma Certidão do indivíduo";
                }
                if (isInvalid($scope.paciente.celular) && isInvalid($scope.paciente.telefone) && $scope.paciente.situacao != 3) {
                    throw "Informe o Celular ou Telefone para Contato";
                }
        ...
        $scope.validarCnsExistenteDominio = function() {
            var deferred = $q.defer();

            if (invalidToEmpty($scope.paciente.numeroCartao).length > 0 && CNS.isValid($scope.paciente.numeroCartao)) {
                var sql = "SELECT nome FROM dominioPaciente where cns = ?";
                if (isValid($scope.paciente.id) || isValid($scope.paciente.codigoSistema)) {
                    sql += " AND (dominioPaciente.codigoInterno <> " + $scope.paciente.id;
                    // sql += " OR (dominioPaciente.codigoInterno is null AND  dominioPaciente.codigoSistema is null))";
                    sql += " OR (dominioPaciente.codigoInterno is null AND dominioPaciente.codigoSistema <> " + $scope.paciente.codigoSistema + "))";
            ...
            if (invalidToEmpty($scope.paciente.numeroCartao).length > 0 && CNS.isValid($scope.paciente.numeroCartao)) {
                ...
                DB.query(sql, [$scope.paciente.numeroCartao]).then(function(res) {
                    ...
                    if (isValid(paciente)) {
                        ...
                        $scope.validarCnsExistenteDominio().then(function() {
                            deferred.resolve();
                        }, function(msgErro) {
        ...
        $scope.validarCpfExistente = function() {
            ...
            if (invalidToEmpty($scope.documentosBind.numeroCpf).length > 0 && CPF.isValid($scope.documentosBind.numeroCpf)) {
                var sql = "SELECT nome FROM dominioPaciente where cpf = ?";
                if (isValid($scope.paciente.id) || isValid($scope.paciente.codigoSistema)) {
                    sql += " AND (dominioPaciente.codigoInterno <> " + $scope.paciente.id;
                    // sql += " OR (dominioPaciente.codigoInterno is null AND  dominioPaciente.codigoSistema is null))";
                    sql += " OR (dominioPaciente.codigoInterno is null AND dominioPaciente.codigoSistema <> " + $scope.paciente.codigoSistema + "))";
        ...
        $scope.err = function(tx, result) {
            $scope.messagePopup(result.message);
        };
        ...
        $scope.resolverCns = function(tx, idPaciente) {
            ...
            } else if (isValid($scope.paciente.numeroCartao)) {
                //Ativa a trigger de version
                parametros.push('N');
                parametros.push($scope.paciente.cnsId);
                sql = "UPDATE cns SET excluido= ? WHERE id = ?";
                tx.executeSql(sql, parametros, function(tx, result) {
                    //Apenas atualiza e continua
                    $scope.resolverCpf(tx, idPaciente);
                }, $scope.err);
        ...
        $scope.salvarPacienteEsus = function(tx, idPaciente) {
            ...
            tx.executeSql("SELECT codigoPaciente, dataCadastro, situacaoConjugal, codigoCbo, frequentaEscola, nivelEscolaridade," +
                ...
                " possuiSofrimentoPsiquicoGrave, utilizaProtese, proteseAuditiva, proteseMembrosSuperiores, proteseMembrosInferiores, proteseCadeiraRodas, proteseOutros," +
                " parentescoResponsavel, codigoUsuario " +
                " FROM pacienteEsus where codigoPaciente = ? ", [idPaciente],
                function(tx, result) {
                    var pacienteEsus = DB.fetch(result)
        ...
        $scope.salvarDominioPaciente = function(tx, idNovo, idAtualizacao) {
            ...
            binding.push($scope.paciente.numeroCartao);
            binding.push($scope.documentosBind.numeroCpf);
            binding.push($scope.paciente.sexo);
            var keyword = $scope.paciente.nome + ' ' + ' ' + $scope.paciente.numeroCartao + ' ' + $scope.paciente.nomeMae + ' ' + $scope.documentosBind.numeroCpf;
            binding.push(keyword.latinise());
            binding.push(idNovo || idAtualizacao);
            binding.push($scope.paciente.situacao);
            binding.push($scope.paciente.codigoSistema);

            if (!isInvalid(idNovo) && isInvalid($scope.paciente.codigoSistema)) {
                registroPaciente = "INSERT INTO dominioPaciente " +
                    " (nome, dataNascimento, nomeMae, cns, cpf, sexo, keyword, codigoInterno, ativo, codigoSistema)" +
                    "VALUES(upper(?), ?, upper(?), ?, ?, ?, ?, ?, ?, ?);";
            } else {
                registroPaciente = "UPDATE dominioPaciente " +
                    " SET nome = ?, dataNascimento = ?, nomeMae = ?, cns = ?, cpf = ?, sexo = ?, keyword = ?, codigoInterno = ?, ativo = ?, codigoSistema = ?  ";
                if (!isInvalid(idAtualizacao)) {
                    registroPaciente += "WHERE codigoInterno = ?";
                    binding.push(idAtualizacao);
                } else {
                    registroPaciente += "WHERE codigoSistema = ?";
                    binding.push($scope.paciente.codigoSistema);
                }
            }

            tx.executeSql(registroPaciente, binding, function(tx, result) {}, $scope.err);
        ...
        $scope.salvarPaciente = function(tx) {
            ...
            if (isInvalid($scope.paciente.id)) {
                registroPaciente = "INSERT INTO paciente " +
                    ...
                    " codigoPais, codigoCidade, resideDesde, rendaFamiliar, codigoUsuario, rg, cpf, situacao, nis, prontuario, motivoExclusao, codigoEtniaIndigena, keyword, codigoSistema)" +
            ...
            var keyword = $scope.paciente.nome + ' ' + $scope.paciente.nomeMae + ' ' + $scope.documentosBind.numeroCpf;
            binding.push(keyword.latinise());
            if (!isInvalid($scope.paciente.id)) {
            ...
            tx.executeSql(registroPaciente, binding, function(tx, result) {
                ...
                // if (isInvalid($scope.paciente.id)) {
                // var isNovo = !!result.insertId
                $scope.salvarDominioPaciente(tx, result.insertId, $scope.paciente.id);
                // }
...
controllers.controller('tabIndIdentificacaoCtrl', ['$scope', '$ionicModal', 'DB', '$timeout', '$celkScroll', '$cordovaDatePicker',
    function($scope, $ionicModal, DB, $timeout, $celkScroll, $cordovaDatePicker) {
        ...
        $scope.B.watchRemover = $scope.$watch(function(scope) {
                return scope.$parent.paciente.flagResponsavelFamiliar;
            },
        ...
        $scope.B.watchRemoverSituacao = $scope.$watch(function(scope) {
                return scope.$parent.paciente.situacao;
            },
            function(newValue, oldValue, scope) {
                if (newValue == 0) {
...
controllers.controller('tabCondicoesSaudeCtrl', ['$scope', '$ionicModal',
    function($scope, $ionicModal) {
        ...
        $scope.condicoesSaudeBind.exibirGestante = function() {
            if (isValid($scope.paciente.dataNascimento)) {
                var timeDiff = Math.abs($scope.paciente.dataNascimento.getTime() - new Date().getTime());
                var diffDays = Math.ceil(timeDiff / (1000 * 3600 * 24));
                if ($scope.$parent.paciente.sexo === 'F' && diffDays > 3285) { //nove anos de idade minima

                    return true;
                }
            }
            $scope.$parent.pacienteEsus.estaGestante = 0;
            $scope.$parent.pacienteEsus.maternidadeReferencia = null;
...
controllers.controller('tabDocumentosCtrl', ['$scope', '$ionicModal', 'DB', '$timeout', '$celkScroll', '$cordovaDatePicker',
    function($scope, $ionicModal, DB, $timeout, $celkScroll, $cordovaDatePicker) {
        ...
        $scope.watchRemover = $scope.$watch(function(scope) {
                return scope.$parent.documentosBind.numeroMatriculaCertidao;
            },
            function(newValue, oldValue, scope) {
                if (isValid(newValue)) {
