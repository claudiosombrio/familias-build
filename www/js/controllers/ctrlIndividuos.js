controllers.controller('individuosCtrl', ['$scope', '$state', 'DB', '$timeout', '$celkScroll', '$cordovaDatePicker',
    function ($scope, $state, DB, $timeout, $celkScroll, $cordovaDatePicker) {
        ...
        $scope.loadMoreData = function () {
            ...
            var params = [];
            var sql = "SELECT u.sexo, u.id, u.situacao, upper(u.nome) AS nome, upper(u.nomeMae) AS nomeMae, strftime('%d/%m/%Y',u.dataNascimento, 'unixepoch') AS dataNascimento"+
                    " FROM paciente AS u LEFT JOIN cns AS c on c.codigoPaciente = u.id ";
            if(isValid(filtroCodigo)){
                sql += " WHERE u.codigoSistema = ? ";
                params.push(filtroCodigo);
            }else{
                sql += " WHERE (u.keyword like ? OR c.numeroCartao like ?)";
                params.push(filtro);
                params.push(filtro);
            }
