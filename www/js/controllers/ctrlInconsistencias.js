controllers.controller('inconsistenciasCtrl', ['$scope', '$state', 'DB',
    function ($scope, $state, DB) {
        ...
        $scope.editarIndividuo = function (individuo) {
            $state.go('cadastroIndividuos.tabIdentificacao', {idIndividuo: individuo.id});
        };
        ...
        $scope.carregarIndividuos = function () {
            DB.query("SELECT ep.mensagem, p.id, upper(p.nome) AS nome, upper(p.nomeMae) AS nomeMae, strftime('%d/%m/%Y',p.dataNascimento, 'unixepoch') AS dataNascimento"+
        ...
        $scope.carregarDomicilios = function () {
            DB.query("SELECT ed.mensagem, d.id, d.codigoSistema, upper(e.logradouro) AS logradouro, upper(e.complementoLogradouro) AS complementoLogradouro, e.numeroLogradouro, upper(e.bairro) AS bairro, d.numeroFamilia"+
