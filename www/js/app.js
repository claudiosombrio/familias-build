...
var app = angular.module('starter', ['ngResource', 'ui.mask', 'ngCpfCns', 'ionic', 'starter.controllers', 'starter.services', 'pasvaz.bindonce', 'starter.filters', 'starter.directives', 'ngCordova']);
...
var calcSinc = function(data) {
    console.log("Dias Sincronização");
    if (data) {
        window.localStorage.sync = undefined;
        window.localStorage.diassinc = data;
    }

    if (window.localStorage.diassinc) {
        var today = new Date();
        var oneDay = 1000 * 3600 * 24;
        //add sync days
        var syncDays;
        if ('undefined' !== window.localStorage.sync) {
            syncDays = window.localStorage.sync;
        } else {
            syncDays = new Date().setDate(new Date().getDate() + parseInt(window.localStorage.diassinc));
        }
        window.localStorage.sync = syncDays;
        var abs = syncDays - today.getTime();
        var diffDays = Math.ceil(abs / (oneDay))
        window.localStorage.datasinc = diffDays;
    }
}
...
app.run(function($ionicPlatform, DB, FS, $rootScope, $ionicConfig, $cordovaDevice, $cordovaSplashscreen, $cordovaFile, $cordovaGeolocation) {
    $ionicPlatform.ready(function() {
        ...
        DB.init();
        FS.init();
        $rootScope.tituloPrincipal = 'GEM Famílias - MA';
        ...
        $cordovaFile.removeFile('celk/familias/familias.apk').then(function() {}, function(err) {
...
app.config(function($stateProvider, $urlRouterProvider) {
    $stateProvider
        ...
        .state('cadastroVisitas.tabPesquisa', {
            url: '/cadastroVisitaTabs/tabPesquisa',
            views: {
                'tab-Pesquisa': {
                    templateUrl: 'templates/cadastroVisitaTabs/tabPesquisa.html',
                    controller: 'tabPesquisaCtrl'
                }
            }
        })
    ...
    if (url === null) {
        ...
        //Utilizado para manter compatibilidade na migracao dos servidores de veneza
        if (url.indexOf("veneza1") > -1) {
            url = url.replace(/veneza1/g, 'veneza');
